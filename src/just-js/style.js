...
const style = (text, opt) => {
  ...
  switch (currStyle.align) {
    case Align.LEFT:
      currText = text.split('\n').map(line => (opt.foreground ?? '').concat(opt.background ?? '') //aplly colours
        .concat(line.padEnd(currStyle.width, ' '), // align left
          (opt.background || opt.foreground ? ansi.style.reset : '')) // reset colours
      ).join('\n')
      currTextHeight = currText.split('\n').length;
      currText = currText.concat(`\n${opt.background ?? ''}${' '.repeat(currStyle.width)}${opt.background ? ansi.style.reset : ''}` // apply background colour
        .repeat(currStyle.height - currTextHeight)); // add blank lines below the text to attain the opt.height
      break;
    case Align.RIGHT:
      currText = text.split('\n').map(line => (opt.foreground ?? '').concat(opt.background ?? '') //apply colours
        .concat(line.padStart(currStyle.width, ' '), // align right
          (opt.background || opt.foreground ? ansi.style.reset : '')) // aplly colours
      ).join('\n')
      currTextHeight = currText.split('\n').length;
      currText = currText.concat(`\n${opt.background ?? ''}${' '.repeat(currStyle.width)}${opt.background ? ansi.style.reset : ''}` // apply background colour
        .repeat(currStyle.height - currTextHeight)); // add blank lines below the text
      break;
    case Align.CENTER:
      const hGap = Math.floor((currStyle.width - text.split('\n').reduce((length, line) => line.length > length ? line.length : length, 0)) / 2);

      currText = text.split('\n').map(line => `${opt.foreground ?? ''}${opt.background ?? ''}` // add colour to text
        .concat(' '.repeat(hGap), line, ' '.repeat(hGap)// add blank spaces left and right of the text
          , opt.foreground || opt.background ? ansi.style.reset : '')) // reset colour
        .join('\n')
      ...
      currText = currText.concat(`\n${(opt.background ?? '') // add background colour to blank lines
        ...
        .repeat(getLineLength(currText))}` // blank line with length equal to the text line
    ...
    case Align.MIDDLE:
      ...
      currText = text.split('\n').map(line => `${opt.foreground ?? ''}${opt.background ?? ''}`.concat(line.padEnd(currStyle.width, ' '), opt.background || opt.foreground ? ansi.style.reset : '')).join('\n')// align left

      currText = (`${opt.background ?? ''}${' '.repeat(currStyle.width)}${opt.background ? ansi.style.reset : ''}\n` // blank lines above
        .repeat(vGap).concat(currText, // text lines
...
const getBorders = borderType => {
  ...
  return { borderX, borderY, cornerLowerLeft, cornerUpperLeft, cornerLowerRight, cornerUpperRight };
}

const style2 = (text, options) => {
  const opt = { ...DEFAULT_STYLE, ...options };
  opt.width = (() => {
    const currentWidth = opt.width - getLineLength(text) + opt.marginRight + opt.marginLeft + opt.paddingRight + opt.paddingLeft + 2;
    const diff = opt.width - currentWidth;
    if (diff > 0) return opt.width - diff + 1;
    return Math.abs(currentWidth);
  })()
  const { borderX, borderY, cornerUpperRight, cornerUpperLeft, cornerLowerRight, cornerLowerLeft } = getBorders(opt.border);
  ...
  const hGapCount = opt.width - getLineLength(text) >= 0 ? opt.width - getLineLength(text) : 0;
  const hAlignmentGap = ' '.repeat(hGapCount);
  const alignRight = opt.align === Align.RIGHT ? hAlignmentGap : '';
  const alignLeft = opt.align === Align.LEFT || (opt.align !== Align.RIGHT && opt.align !== Align.CENTER) ? hAlignmentGap : '';
  const alignCenter = opt.align === Align.CENTER ? ' '.repeat(hGapCount / 2) : '';
  ...
  const lineLengthWithoutBorder = lineLength - opt.marginLeft - opt.marginRight - (opt.border === Border.NONE ? 0 : 2);
  const blankLineWithBorder = `${marginLeft}${borderY}${' '.repeat(lineLengthWithoutBorder)}${borderY}${marginRight}`;
  ...
  const bottomAlignmentGap = opt.align === Align.BOTTOM ? blankLineWithBorder.concat('\n').repeat(opt.height - noOfLines) : '';
  const middleAlignmentGap = opt.align === Align.MIDDLE ? blankLineWithBorder.concat('\n').repeat((opt.height - noOfLines) / 2) : '';

  const borderH = borderX.repeat(lineLengthWithoutBorder);
  const marginTop = opt.marginTop ? ' '.repeat(lineLength).concat('\n').repeat(opt.marginTop) : '';
  const topBorder = `${marginLeft}${cornerUpperLeft}${borderH}${cornerUpperRight}${marginRight}\n`
  const paddingTop = opt.paddingTop ? blankLineWithBorder.concat('\n').repeat(opt.paddingTop) : '';
  const styledTop = `${marginTop}${topBorder}${paddingTop}${bottomAlignmentGap}${middleAlignmentGap}`
  ...
  const styledBotom = `${middleAlignmentGap}${topAlignmentGap}${paddingBottom}${bottomBorder}${marginBottom}`;

  const result =
    `${styledTop}${styledText}${styledBotom}`;
